...
class modLPPO(PPO):
    ...
    def __init__(self, policy, env, n_objectives, eta_values: List[Union[float, Schedule]] = None, beta_values: List[float] = None,
                 tolerance: Union[float, Schedule] = 0.05, recent_loses_len: int = 50, lagr_momentum:bool=False, *args, **kwargs):
        ...
        self.recent_returns = [deque(maxlen=recent_loses_len) for _ in range(self.n_objectives)]
    ...
    def average_mc_discounted_return(self):
        rewards = self.rollout_buffer.rewards  # (n_steps, n_envs, n_objectives)
        episode_starts = self.rollout_buffer.episode_starts
        gamma = self.gamma

        n_steps, n_envs, n_objectives = rewards.shape
        episode_returns = []

        for env in range(n_envs):
            G = np.zeros(n_objectives, dtype=np.float32)
            for t in reversed(range(n_steps)):
                G = rewards[t, env] + gamma * G
                if episode_starts[t, env]:
                    episode_returns.append(G.copy())
                    G[:] = 0.0

        if len(episode_returns) == 0:
            return np.zeros(n_objectives)

        return np.mean(np.stack(episode_returns, axis=0), axis=0)

    def average_gae_episode_return(self):
        """
        Computes average per-episode GAE-estimated discounted return
        for multi-objective PPO.

        returns shape: (n_steps, n_envs, n_objectives)
        """
        returns = self.rollout_buffer.returns
        episode_starts = self.rollout_buffer.episode_starts

        n_steps, n_envs, n_objectives = returns.shape
    ...
    def train(self):
        ...
        # Compute rollout returns with true MC estimate

        mc_return = self.average_mc_discounted_return()
        lenbuff = len(self.recent_returns[0])
        improvement = [0.0 for i in range(self.n_objectives)]
        tol = self.tolerance if isinstance(self.tolerance, float) else self.tolerance(self._current_progress_remaining)

        for obj in range(self.n_objectives):
            self.recent_returns[obj].append(mc_return[obj])
            self.logger.record(f"train_mo/mc_return_{obj}", mc_return[obj])

            if lenbuff > 1:

                new = np.mean(list(self.recent_returns[obj])[-lenbuff // 2:])
                old = np.mean(list(self.recent_returns[obj])[:lenbuff // 2])
                improvement[obj] = new - old
                self.logger.record(f"train_mo/improvment_on_{obj}", improvement[obj])

                if obj == self.n_objectives - 1:
                    break

                eta = self.eta_values[obj] if not callable(self.eta_values[obj]) else self.eta_values[obj](
                    self._current_progress_remaining)

                if abs(improvement[obj]) > tol * abs(new):
                    # Big enough to move the lagrangian
                    self.mu_values[obj] += eta * - improvement[obj]

                self.mu_values[obj] = max(0, self.mu_values[obj])
